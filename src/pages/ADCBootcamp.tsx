...
const ADCBootcamp = () => {
 ...
 <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-3 heading-display text-gray-900 ">Microsoft ADC Bootcamps</h1>
 ...
 <article key={session.label} className="bg-white rounded-xl p-6 shadow-soft-md border border-gray-100 transition-all duration-300 hover:shadow-soft-lg hover:-translate-y-1.5">
 <div className="flex items-center justify-between mb-4">
 <h2 className="text-xl font-semibold text-gray-900 heading-display">{session.label}</h2>
 <span className="text-sm font-medium bg-gray-900/10 px-3 py-1 rounded-full">{session.badge}</span>
 </div>
 <p className="text-gray-600 text-sm">{session.blurb}</p>
 </article>
 ))}
 </div>
 </section>
 ...
 {fastFacts.map((fact) => (
 <div key={fact.title} className="flex items-start bg-gradient-to-br from-teal-50/60 to-white rounded-xl p-6 border border-teal-100 transition-all duration-300">
 <fact.icon className="h-10 w-10 text-teal-600 mr-4" />
 <div>
 <p className="text-lg font-semibold text-gray-900 ">{fact.title}</p>
 ...
 <div className="bg-white rounded-xl p-8 shadow-soft-md border border-gray-100 ">
 <h3 className="text-2xl font-semibold tracking-tight text-gray-900 mb-4 heading-display">Quick Readiness Checklist</h3>
 <ul className="space-y-3">
 {readinessSteps.map((step) => (
 <li key={step} className="flex items-start text-gray-600 text-sm">
 ...
 className="px-5 py-2 rounded-xl border border-teal-200 text-gray-700 transition-all duration-200 hover:shadow-soft-md"
 >
